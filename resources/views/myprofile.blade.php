        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form action="{{ url('myprofileedit/'.$profile->user_name) }}" method="POST">
                        @csrf
                        <button type="submit" class="btn bg-blue-500 m-2 p-2">プロフィールを編集</button>
                    </form>
                    <form action="{{ route('userprofile_destroy', ['userprofile'=>$profile->user_name]) }}" method="post">
                        @method('DELETE')
                        @csrf
                        <button type="submit" class="btn bg-blue-500 m-2 p-2">プロフィールを削除</button>
                    </form>
                        ...
                        <div>{{ 'カレンダーの共有リンク：' }}
                            <a href="{{ $profile->calendar }}">{{ $profile->calendar }}</a>
                        </div>
